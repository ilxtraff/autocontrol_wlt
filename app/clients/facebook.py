...
class FacebookError(RuntimeError):
    ...
    def hint(self) -> str:
        ...
        if self.code in (1, 2):
            # #1/#2 — «неизвестная/временная ошибка». На /me это почти всегда
            # мёртвый токен либо сессия, которую Facebook не принял с этого IP.
            return (
                "Facebook отклонил запрос без конкретной причины. Обычно это "
                "мёртвый токен или сессия, отклонённая с IP прокси. Снимите "
                "свежий мультитокен и убедитесь, что прокси в стране аккаунта"
            )
        return ""
